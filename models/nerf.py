...
class NeRF(nn.Module):
    def __init__(self, typ,
        ...
        typ: if 'coarse', refers to coarse model. else, refers to fine model.
        ...
        """
        don't use any embeddings if the network is coarse, since the primary aim for coarse network is to guide the 
        sampling process by the generated density
        """
        ...
        # xyz encoding layers, refers to the orange block in Figure 3 (NeRF-W)
        for i in range(D):
            if i == 0:
